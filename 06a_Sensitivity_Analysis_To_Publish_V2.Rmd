...
input.path = paste0("C:/Users/",pnnl.user,"/PNNL/Core Richland and Sequim Lab-Field Team - Documents/Data Generation and Files/ECA/INC/03_ProcessedData/Raw_DO_by_Min/EC_INC_Raw_DO_By_Min_ReadyForBoye_2024-05-30.csv")

output.path = paste0("C:/Users/",pnnl.user,"/PNNL/Core Richland and Sequim Lab-Field Team - Documents/Data Generation and Files/ECA/Optode multi reactor/Optode_multi_reactor_incubation/rates/ReadyForBoye/")
...
#write.csv(respiration,paste0(output.path,"ECA_Sediment_Incubations_Respiration_Rates_",Sys.Date(),".csv"), row.names = F)
...
solid <- read.csv(paste0("C:/Users/",pnnl.user,"/PNNL/Core Richland and Sequim Lab-Field Team - Documents/Data Generation and Files/ECA/INC/03_ProcessedData/EC_Drying_Masses_Summary_ReadyForBoye_on_2024-04-26.csv"))
...
all.samples <-  left_join(removed, respiration, by = "Sample_Name") %>% 
  ...
  mutate(Treat = case_when(grepl("W",rep)~"Wet",
                           grepl("D", rep) ~"Dry")) %>% 
  mutate(Methods_Deviation.theoretical = if_else(number_of_points == 2 & `2_min_concentration` <= 5.5, "RATE_005", NA_character_)) %>% 
...
boye <- all.samples %>% 
  ...
  mutate(`2_min_concentration` = round(`2_min_concentration`, 4)) %>% 
  mutate(slope_of_the_regression = if_else(slope_of_the_regression >0,0,slope_of_the_regression)) %>%
  mutate(rate_mg_per_L_per_min = if_else(slope_of_the_regression >=0,0,rate_mg_per_L_per_min )) %>%
  mutate(rate_mg_per_L_per_h = if_else(slope_of_the_regression>=0,0,rate_mg_per_L_per_h)) %>% 
  mutate(rate_mg_per_L_per_h = if_else(is.na(rate_mg_per_L_per_h), 9999, rate_mg_per_L_per_h)) %>%
  mutate(R_squared = if_else(is.na(R_squared), -9999, R_squared)) %>% 
  mutate(R_squared_adj = if_else(is.na(R_squared_adj), -9999, R_squared_adj)) %>% 
  ...
  mutate(number_of_points = if_else(is.na(number_of_points), as.numeric(-9999), as.numeric(number_of_points))) %>% 
  mutate(no_points_rem = if_else(is.na(no_points_rem), -9999, no_points_rem)) %>% 
  ...
  dplyr::select(c(Sample_Name, rate_mg_per_L_per_h, R_squared, R_squared_adj, `p_value`, total_incubation_time_min, number_of_points, no_points_rem, `2_min_concentration`, Methods_Deviation, kit, Treat)) %>% 
  mutate(rate_mg_per_L_per_h = rate_mg_per_L_per_h * -1) %>%
  relocate(Methods_Deviation, .after = `2_min_concentration`) %>% 
  rename(Respiration_Rate_mg_DO_per_L_per_H = rate_mg_per_L_per_h) %>% 
  rename(Respiration_R_Squared = R_squared) %>% 
  rename(Respiration_R_Squared_Adj = R_squared_adj) %>% 
  rename(Respiration_p_value = p_value) %>% 
  rename(Total_Incubation_Time_Min = total_incubation_time_min) %>% 
  rename(Number_Points_In_Respiration_Regression = number_of_points) %>% 
  rename(Number_Points_Removed_Respiration_Regression = no_points_rem) %>% 
  rename(DO_Concentration_At_Incubation_Time_Zero = `2_min_concentration`) %>% 
  ...
  mutate(Respiration_Rate_mg_DO_per_L_per_H = as.numeric(Respiration_Rate_mg_DO_per_L_per_H)) %>%
  mutate(Respiration_Rate_mg_DO_per_kg_per_H = if_else(Respiration_Rate_mg_DO_per_L_per_H > -9999, Respiration_Rate_mg_DO_per_L_per_H * (Incubation_Water_Mass_g/Dry_Sediment_Mass_g), -9999)) %>% 
  mutate(Respiration_Rate_mg_DO_per_kg_per_H = if_else(Incubation_Water_Mass_g == -9999, -9999, Respiration_Rate_mg_DO_per_kg_per_H)) %>% 
  relocate(Respiration_Rate_mg_DO_per_kg_per_H, .after = Respiration_Rate_mg_DO_per_L_per_H) %>% 
  relocate(Material, .after = Sample_Name) %>% 
  mutate(Respiration_Rate_mg_DO_per_kg_per_H = round(Respiration_Rate_mg_DO_per_kg_per_H, 3)) %>% 
  relocate(SpC, .after = Material) %>% 
  relocate(pH, .after = SpC) %>% 
  relocate(Temp, .after = pH) %>% 
  relocate(Methods_Deviation.x, .before = Methods_Deviation.y) %>% 
  dplyr::select(-c(Initial_Water_mass_g, Final_Water_mass_g, Incubation_Water_Mass_g, Dry_Sediment_Mass_g)) # rename(Methods_Deviation = Methods_Deviation.x)
...
## Do not end up using these because we published as medians
...
slope.final <- as.data.frame(matrix(NA, ncol = 10, nrow =1))

colnames(slope.final) = c("slope.temp","Sample_Name", "kit_treat", "Respiration_Rate_mg_DO_per_L_per_H", "Mean_Slope_All_L", "Mean_Slope_All_kg", "cv_before_removal_L", "cv_before_removal_kg", "cv_after_removal", "Mean_Slope_Removed")
...
write.csv(eca_respiration_all_final, "C:/Users/laan208/PNNL/Core Richland and Sequim Lab-Field Team - Documents/Data Generation and Files/ECA/INC/03_ProcessedData/ECA_Sediment_Incubations_Respiration_Rates_ReadyForBoye_2024-05-30.csv", row.names = F)

```

```{r Summary File }
## DO NOT USE, USED MEDIANS INSTEAD
...
eca_summary_join = left_join(eca_summary_all, eca_summary_best, by = "Sample") %>% 
  rename(Mean_Respiration_Rate_mg_DO_per_L_per_H = Respiration_Rate_mg_DO_per_L_per_H_mean) %>% 
    rename(SD_Respiration_Rate_mg_DO_per_L_per_H = Respiration_Rate_mg_DO_per_L_per_H_sd) %>% 
  rename(Mean_Respiration_Rate_mg_DO_per_kg_per_H = Respiration_Rate_mg_DO_per_kg_per_H_mean) %>% 
    rename(SD_Respiration_Rate_mg_DO_per_kg_per_H = Respiration_Rate_mg_DO_per_kg_per_H_sd) %>% 
  rename(Best_Mean_Respiration_Rate_mg_DO_per_L_per_H = Respiration_Rate_mg_DO_per_L_per_H_best_mean) %>% 
   rename(Best_SD_Respiration_Rate_mg_DO_per_L_per_H = Respiration_Rate_mg_DO_per_L_per_H_best_sd) %>% 
  rename(Best_Mean_Respiration_Rate_mg_DO_per_kg_per_H = Respiration_Rate_mg_DO_per_kg_per_H_best_mean) %>% 
rename(Best_SD_Respiration_Rate_mg_DO_per_kg_per_H = Respiration_Rate_mg_DO_per_kg_per_H_best_sd) %>% 
  ...
  dplyr::select(c(Sample_Name, Material, Mean_SpC, SD_SpC, Mean_pH, SD_pH, Mean_Temp, SD_Temp, Mean_Respiration_Rate_mg_DO_per_L_per_H, SD_Respiration_Rate_mg_DO_per_L_per_H, Mean_Respiration_Rate_mg_DO_per_kg_per_H, SD_Respiration_Rate_mg_DO_per_kg_per_H, Best_Mean_Respiration_Rate_mg_DO_per_L_per_H, Best_SD_Respiration_Rate_mg_DO_per_L_per_H, Best_Mean_Respiration_Rate_mg_DO_per_kg_per_H, Best_SD_Respiration_Rate_mg_DO_per_kg_per_H))

write.csv(eca_summary_join, "C:/Users/laan208/PNNL/Core Richland and Sequim Lab-Field Team - Documents/Data Generation and Files/ECA/INC/03_ProcessedData/ECA_Sediment_Incubations_Respiration_Rates_Summary_ReadyForBoye_2024-03-05.csv", row.names = F)
