...
fs_cube = ggplot(cube_effect_data_corr, aes(x = Cube_Fine_Sand, y = Cube_Effect_Size)) +
  ...
  stat_cor(data = cube_effect_data_corr, label.x = 0.9, label.y = 10, size = 4, digits = 2, aes(label = paste(..p.label..)))+
  stat_poly_line(data = cube_effect_data_corr, se = FALSE)+
...
lasso = cv.glmnet(xvars, yvar, alpha = 1, 
                  standardize = FALSE, standardize.response = FALSE, intercept = FALSE, nfolds = 3)
