...
solid <- read.csv(paste0("C:/Users/",pnnl.user,"/PNNL/Core Richland and Sequim Lab-Field Team - Documents/Data Generation and Files/ECA/INC/03_ProcessedData/ECA_Drying_Masses_Summary_ReadyForBoye_01-17-2024.csv"))
...
## Get respiration data in Boye format. Join solid:solution ratio and calculate mg/kg values
boye <- all.samples %>% 
...
mutate(rate_mg_per_L_per_h = if_else(slope_of_the_regression>=0,0,rate_mg_per_L_per_h)) %>% 
  ...
  mutate(Respiration_Rate_mg_DO_per_L_per_H = as.numeric(Respiration_Rate_mg_DO_per_L_per_H)) %>%
     group_by(kit, Treat) %>% 
  mutate(cv_before_removal = abs(sd(Respiration_Rate_mg_DO_per_L_per_H)/mean(Respiration_Rate_mg_DO_per_L_per_H))*100) %>% 
   mutate(Methods_Deviation.processed = if_else(cv_before_removal >= 10, "RATE_CV_010", NA_character_)) %>%
  fill(Methods_Deviation.processed, .direction = "downup") %>% 
  ungroup() %>%
   mutate(Methods_Deviation = if_else(is.na(Methods_Deviation.processed), Methods_Deviation, paste(Methods_Deviation, Methods_Deviation.processed, sep = "; "))) %>%
  mutate_all(~gsub("N/A; |N/A ; ", "",.)) %>% 
  dplyr::select(-c(kit, Treat, cv_before_removal, Methods_Deviation.processed)) %>% 
 ...
 mutate(Respiration_Rate_mg_DO_per_kg_per_H = if_else(Incubation_Water_Mass_g == -9999, -9999, Respiration_Rate_mg_DO_per_kg_per_H)) %>% 
  ...
  dplyr::select(-c(Initial_Water_Mass_g, Final_Water_Mass_g, Incubation_Water_Mass_g, Dry_Sediment_Mass_g, Methods_Deviation.y, flag)) %>% 
  rename(Methods_Deviation = Methods_Deviation.x)

#write.csv(boye,paste0(output.path,"ECA_Sediment_Incubations_Respiration_Rates_ReadyForBoye",Sys.Date(),".csv"), row.names = F)
...
## Remove samples with no Rates (-9999)
slope.outliers <- boye %>% 
  separate(Sample_Name, c("Kit", "Rep"), sep = "-", remove = FALSE) %>% 
  mutate(Treat = case_when(grepl("W", Rep)~"Wet",
                           grepl("D", Rep) ~"Dry")) %>% 
  ...
  mutate(Respiration_Rate_mg_DO_per_L_per_H = as.numeric(Respiration_Rate_mg_DO_per_L_per_H)) %>%
  group_by(Kit, Treat) %>% 
  mutate(Mean_Slope_All = mean(Respiration_Rate_mg_DO_per_L_per_H)) %>% 
  mutate(cv_before_removal = abs((sd(Respiration_Rate_mg_DO_per_L_per_H)/mean(Respiration_Rate_mg_DO_per_L_per_H))*100)) %>% 
  ungroup() %>% 
  dplyr::select(c(Sample_Name, Material, Respiration_Rate_mg_DO_per_L_per_H, Methods_Deviation, Mean_Slope_All, cv_before_removal, kit_treat)) 

slope.outliers$flag <- NA

 slope.final <- as.data.frame(matrix(NA, ncol = 10, nrow =1))

 colnames(slope.final) = c("slope.temp","Sample_Name", "kit_treat", "Material", "Mean_Slope_All","cv_before_removal", "cv_after_removal", "Mean_Slope_Removed","flag", "Methods_Deviation")
...
slope.final.flag <- slope.final %>% 
  mutate(flag = if_else(cv_before_removal < cv_after_removal, "Issue in dropping sample", "N/A")) %>% 
rename(Respiration_Rate_mg_DO_per_L_per_H = slope.temp)


## Fix boye file to reflect sites/treatments where removing samples doesn't decrease CV

final_boye <- left_join(boye, slope.final.flag, by = c( "Sample_Name", "Material", "Methods_Deviation")) %>% 
  rename(Respiration_Rate_mg_DO_per_L_per_H = Respiration_Rate_mg_DO_per_L_per_H.x) %>% 
  mutate(flag_removal = if_else(is.na(Respiration_Rate_mg_DO_per_L_per_H.y), "Outlier to remove", "N/A"))
...
```{r Data without Removals}

data_clean_no_removals <- slope.outliers %>% 
  mutate(Respiration_Rate_mg_DO_per_L_per_H = abs(Respiration_Rate_mg_DO_per_L_per_H)) %>% 
  mutate(Mean_Slope_All = abs(Mean_Slope_All)) %>% 
  separate(kit_treat, c("EC", "kit", "INC", "Treat"), sep = "_") %>% 
  dplyr::select(-c(EC, INC))

#cv_before <- slope.outliers %>% 
  #distinct(kit_treat, .keep_all = TRUE)

#cv_before_plot <- ggplot(cv_before, aes(x = cv_before_removal)) + 
  #geom_histogram()

wet_no <- ggplot(subset(data_clean_no_removals, Treat %in% "Wet"), aes(x = Respiration_Rate_mg_DO_per_L_per_H)) +
  geom_histogram()+
  ggtitle("Wet Rates, No Removals")+
  theme(strip.text = element_text(
    size = 4))

dry_no <- ggplot(subset(data_clean_no_removals, Treat %in% "Dry"), aes(x = Respiration_Rate_mg_DO_per_L_per_H)) +
  geom_histogram()+
  ggtitle("Dry Rates, No Removals")+
  theme(strip.text = element_text(
    size = 4))

no_theoretical <- data_clean_no_removals %>% 
  filter(!grepl("RATE_005", Methods_Deviation))
  
removed_theoretical_dry <- ggplot(subset(no_theoretical, Treat %in% "Dry"), aes(x = Respiration_Rate_mg_DO_per_L_per_H)) +
  geom_histogram()+
  ggtitle("Dry Rates, Theoretical Removed")+
  theme(strip.text = element_text(
    size = 4))

removed_theoretical_wet <- ggplot(subset(no_theoretical, Treat %in% "Wet"), aes(x = Respiration_Rate_mg_DO_per_L_per_H)) +
  geom_histogram()+
  ggtitle("Wet Rates, Theoretical Removed")+
  theme(strip.text = element_text(
    size = 4))

## Calculate effect Size only if we have n = 4 for Wet and Dry rates
## 11 and 12 only have dry rates
data_clean_effect <- data_clean_no_removals %>% 
  group_by(kit, Treat) %>% 
  filter(n() >= 4) %>% 
  ungroup() %>% 
  group_by(kit) %>% 
...
no_theoretical_effect <- no_theoretical %>% 
  group_by(kit, Treat) %>% 
  filter(n() >= 4) %>% 
  ungroup() %>% 
  group_by(kit) %>% 
...
removed_theoretical_effect <- ggplot(no_theoretical_effect, aes(x = effect)) + 
  geom_histogram() +
  ggtitle("Effect Size, No Theoretical")+
  theme(strip.text = element_text(
    size = 4))
...
cv_after <- slope.final.flag %>% 
  distinct(kit_treat, .keep_all = TRUE) 

cv_after_plot <- ggplot(slope.final.flag, aes(x = cv_after_removal)) + 
  geom_histogram()

data_clean_removals = slope.final.flag %>%
  mutate(Mean_Slope_Removed = abs(Mean_Slope_Removed)) %>%
  mutate(Respiration_Rate_mg_DO_per_L_per_H = abs(Respiration_Rate_mg_DO_per_L_per_H)) %>% 
  dplyr::select(c(Sample_Name,Respiration_Rate_mg_DO_per_L_per_H, Mean_Slope_Removed)) %>%
...
removed <- data_clean_removals %>% 
  dplyr::select(c(Sample_Name, Respiration_Rate_mg_DO_per_L_per_H))

average <- data_clean_removals %>% 
  distinct(kit_treat, .keep_all = TRUE) %>% 
  dplyr::select(c(ID, Treat, Mean_Slope_Removed))

#write.csv(average,paste0(output.path,"ECA_Sediment_Incubations_Removed_Respiration_Rates_Averages",Sys.Date(),".csv"), row.names = F)

#write.csv(removed,paste0(output.path,"ECA_Sediment_Incubations_Removed_Respiration_Rates_",Sys.Date(),".csv"), row.names = F)


wet_rem <- ggplot(subset(data_clean_removals, Treat %in% "Wet"), aes(x = Respiration_Rate_mg_DO_per_L_per_H)) +
  geom_histogram()+
  ggtitle("Wet Rates, Removals")+
  theme(strip.text = element_text(
    size = 4))

dry_rem <- ggplot(subset(data_clean_removals, Treat %in% "Dry"), aes(x = Respiration_Rate_mg_DO_per_L_per_H)) +
  geom_histogram()+
...
kits <- boye %>%
  separate(Sample_Name, c("ID", "Rep"), sep = "-", remove = FALSE) %>% 
  distinct(ID, .keep_all = TRUE) %>% 
  filter(!is.na(ID)) %>% 
  dplyr::select(c(ID, Methods_Deviation, Material))

effect_boye <- left_join(kits, data_clean_rem_effect, by = "ID") %>% 
  dplyr::select(c(ID, Material, effect, Methods_Deviation)) %>% 
  rename(Effect_Size = effect) %>% 
  rename(Sample_Name = ID) %>% 
  mutate(Effect_Size = if_else(is.na(Effect_Size), -9999, Effect_Size)) %>% 
  mutate(Effect_Size = round(Effect_Size, 3)) %>% 
  mutate(Methods_Deviation = if_else(Effect_Size == -9999, "EFFECT_000", "N/A")) 
  

effect.path <- paste0("C:/Users/",pnnl.user,"/PNNL/Core Richland and Sequim Lab-Field Team - Documents/Data Generation and Files/ECA/Optode multi reactor/Optode_multi_reactor_incubation/rates/ReadyForBoye/")


write.csv(effect_boye, paste0(effect.path,"ECA_Effect_Size_ReadyForBoye_",Sys.Date(),".csv"), row.names = F)
